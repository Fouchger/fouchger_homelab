...
#   - Intended to run on the control plane host.
#
# Environment:
...
# Anchor off this script so it works even when installed without a .git directory.
...
usage() {
  cat <<EOF
Developer authentication helper (control plane)
...
is_tty() { [[ -t 0 && -t 1 ]]; }
is_debian_like() { [[ -r /etc/debian_version ]]; }
...
apt_install() {
  ...
  if [[ $EUID -ne 0 ]]; then
    ...
    error "Need root or sudo to install '${pkg}' via apt."
    return 1
  fi
  apt-get update -y
