...
# Created: 2026-01-23
# Description: Module and library loading for fouchger_homelab.
# Usage:
...
#   - Policy: executable scripts should source only this file (not individual libs).
#     Then call:
#       homelab_load_lib
#       homelab_load_modules   # only if module entrypoints are required
#   - Paths policy: scripts must use variables from lib/paths.sh; never hard-code
#     ~/.config paths or /tmp paths.
# -----------------------------------------------------------------------------
set -Eeuo pipefail
IFS=$'\n\t'
